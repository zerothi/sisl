...
def cell_plot_actions(cell: CellLike = None, show_cell: Literal[False, "box", "axes"] = "box", axes=["x", "y", "z"], 
    name: str = "Unit cell", cell_style={}, dataaxis_1d=None):    
    if show_cell == False:
        cell_plottings = []
    else:
        ...
        ndim = get_ndim(axes)
        z = get_z(ndim)
        cell_plottings = draw_xarray_xy(data=projected_cell_lines, x="x", y="y", z=z, set_axequal=ndim > 1, name=name)
