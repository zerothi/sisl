...
class BandsData(XarrayData):
    ...
    def sanity_check(self, 
        n_spin: Optional[int] = None, nk: Optional[int] = None, nbands: Optional[int] = None,
        klabels: Optional[Sequence[str]] = None, kvals: Optional[Sequence[float]] = None
    ):
        ...
        if n_spin is not None:
            if n_spin == 1:
                assert spin.is_unpolarized, f"Spin in the data is {spin}, but n_spin=1 was expected"
            elif n_spin == 2:
                assert spin.is_polarized, f"Spin in the data is {spin}, but n_spin=2 was expected"
            elif n_spin == 4:
                assert not spin.is_diagonal, f"Spin in the data is {spin}, but n_spin=4 was expected"

        # Check if we have the correct number of spin channels
        if spin.is_polarized:
            assert "spin" in array.dims, f"'spin' dimension missing for polarized spin, existing dimensions: {array.dims}"
            if n_spin is not None:
                assert len(array.spin) == n_spin
        else:
            assert "spin" not in array.dims, f"'spin' dimension present for spin different than polarized, existing dimensions: {array.dims}"
            assert "spin" not in array.coords, f"'spin' coordinate present for spin different than polarized, existing dimensions: {array.dims}"
        ...
        if klabels is not None:
            ...
            assert "ticktext" in array.k.attrs['axis'], "No ticks were found for the k dimension"
            assert tuple(array.k.attrs['axis']['ticktext']) == tuple(klabels), f"Expected labels {klabels} but found {array.k.attrs['axis']['ticktext']}"
        if kvals is not None:
            assert "axis" in array.k.attrs, "No axis specification for the k dimension."
            assert "tickvals" in array.k.attrs['axis'], "No ticks were found for the k dimension"
            assert np.allclose(array.k.attrs['axis']['tickvals'], kvals), f"Expected label values {kvals} but found {array.k.attrs['axis']['tickvals']}"

    @classmethod
    def toy_example(cls, spin: Union[str, int, Spin] = "", n_states: int = 20, nk: int = 30, gap: Optional[float] = None):
        ...
        random_polinomials[..., :n_bands // 2, 0] *= -1 # Make the curvature negative below the gap
        random_polinomials[..., 2] += shift # Shift each polynomial so that bands stack on top of each other

        # Compute bands
        x = np.linspace(0, 1, nk)
        y = np.outer(x ** 2, random_polinomials[..., 0]) + np.outer(x, random_polinomials[..., 1]) + random_polinomials[..., 2].ravel()
    ...
    @new.register
    @classmethod
    def from_fdf(cls, fdf: fdfSileSiesta, bands_file: Union[str, bandsSileSiesta, None] = None):
    ...
    @new.register
    @classmethod
    def from_wfsx(cls, wfsx_file: wfsxSileSiesta, fdf: str, extra_vars=(), need_H=False):
...
def _get_eigenstate_wrapper(k_vals, spin, extra_vars: Sequence[Union[Dict, str]] = (), spin_moments: bool = True):
