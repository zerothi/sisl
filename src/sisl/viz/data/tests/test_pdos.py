...
def test_pdos_from_sisl_H(spin):
    ...
    data.sanity_check(na=2, no=2, n_spin=n_spin, atom_tags=('C',), dos_checksum=checksum)
...
def test_pdos_from_siesta_PDOS(spin, siesta_test_files):
    ...
    data.sanity_check(na=5, no=72, n_spin=n_spin, atom_tags=('Sr', 'Ti', 'O'), dos_checksum=checksum)
...
def test_pdos_from_siesta_wfsx(spin, siesta_test_files):
    ...
    data.sanity_check(na=15, no=195, n_spin=n_spin, atom_tags=('Bi', 'Se'), dos_checksum=checksum)
...
def test_toy_example(spin):
    ...
    data.sanity_check(n_spin=n_spin, na=data.geometry.na, no=data.geometry.no, atom_tags=["C"])
