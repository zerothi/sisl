...
def create_plot_dispatch(function, name):
    ...
    return type(
        ...
        (PlotDispatch, ),
        {"_plot": staticmethod(function), "__doc__": function.__doc__, "__signature__": inspect.signature(function)}
    )
...
def _get_plotting_func(plot_cls, setting_key):
    ...
    _plot.__signature__ = sig.replace(parameters=[p for p in sig.parameters.values() if p.name != setting_key])

    return _plot


def register_plotable(plotable, plot_cls=None, setting_key=None, plotting_func=None, name=None, default=False, plot_handler_attr='plot', **kwargs):
    """
    ...
    for plotable, cls_method in data_source_cls.new.dispatcher.registry.items():
        ...
        for param in list(signature.parameters.values())[1:]:
            if param.kind == param.VAR_KEYWORD:
                ...
                param = param.replace(name=f'data_{param.name}', kind=param.KEYWORD_ONLY, default={})
        ...
        doc += "This function accepts the arguments for creating both the data source and the plot. The following"\
        ...
        try:
            register_plotable(
                plotable, plot_cls=plot_cls,
                plotting_func=_plot, name=name, default=this_default, plot_handler_attr=plot_handler_attr, 
                **kwargs
            )
        except TypeError:
            pass

def register_sile_method(sile_cls, method: str, plot_cls, setting_key, name=None, default=False, plot_handler_attr='plot', **kwargs):
    ...
    params_info = {
        ...
        "replaced_data_args": replaced_data_args,
        "data_var_kwarg": data_var_kwarg,
        "plot_var_kwarg": new_parameters[-1].name if len(new_parameters) > 0 and new_parameters[-1].kind == inspect.Parameter.VAR_KEYWORD else None
    } 
    ...
    def _plot(obj, *args, **kwargs):
        ...
        try:
            ...
            raise TypeError(f"Error while parsing arguments to create the call {method}")
    ...
    doc += f"This function accepts the arguments both for calling {method} and creating the plot. The following"\
    ...
    register_plotable(
        sile_cls, plot_cls=plot_cls,
        plotting_func=_plot, name=name, default=default, plot_handler_attr=plot_handler_attr, 
        **kwargs
    )