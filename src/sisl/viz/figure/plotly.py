...
class PlotlyFigure(Figure):
    ...
    def _init_multiaxis(self, axis, n):
        ...
        layout_axes = [f"{axis}axis{i + 1}" if i > 0 else f"{axis}axis" for i in range(n) ]
        if axis == "x":
            sides = ["bottom", "top"]
        elif axis == "y":
            sides = ["left", "right"]
    ...
    def _iter_multiaxis(self, plot_actions):
        ...
        for i, section_actions in enumerate(plot_actions):
            ...
            active_axes_kwargs = {f"{ax}axis": v.replace("axis", "") for ax, v in active_axes.items()}
            ...
            'buttons': [
                {
                    'label': '▶',
                    'method': 'animate',
                    'args': [None, {"frame": {"duration": int(frame_duration), "redraw": redraw},
    ...
    def init_coloraxis(self, name, cmin=None, cmax=None, cmid=None, colorscale=None, **kwargs):
        ...
        self.update_layout(**{ax_name: {"colorscale": colorscale, "cmin": cmin, "cmax": cmax, "cmid": cmid}})
    ...
    def draw_area_line(self, x, y, name=None, line={}, text=None, dependent_axis=None, row=None, col=None, **kwargs):
        ...
        if dependent_axis is None:
            ...
            x = [*(chunk_x + norms[:, 0] * chunk_spacing), *reversed(chunk_x - norms[:, 0] * chunk_spacing)]
            y = [*(chunk_y + norms[:, 1] * chunk_spacing), *reversed(chunk_y - norms[:, 1] * chunk_spacing)]
    ...
    def draw_ball_3D(self, xyz, size, color="gray", name=None, vertices=15, row=None, col=None, **kwargs):
        self.add_trace({
            'type': 'mesh3d',
            **{key: val for key, val in sphere(center=xyz, r=size, vertices=vertices).items()},
            'alphahull': 0,
            'color': color,
            'showscale': False,
            'name': name,
    ...
    def draw_arrows_3D(self, x, y, z, dxyz, arrowhead_angle=20, arrowhead_scale=0.3, scale: float = 1, row=None, col=None, **kwargs):
