...
def test_get_gap_coords(bands_data):
    ...
    for to_k in ["Gamma", None, "X"]:
        ...
        k, E = get_gap_coords(bands_data, (vb, vb + 1), from_k="Gamma", to_k=to_k, spin=1)
...
def test_draw_gaps(bands_data, E_axis, display_gap):
    ...
    gap_actions = draw_gaps(
        bands_data, gap=display_gap, gap_info=gap_info,
        gap_tol=0.3, gap_color="red", gap_marker={},
        direct_gaps_only=False, custom_gaps=[], E_axis=E_axis
    )
    ...
    assert isinstance(gap_actions, list)
    assert len(gap_actions) == (2 if display_gap else 1) + (1 if spin.is_polarized else 0)
...
def test_style_bands(bands_data):
    ...
    def color(data):
        return xr.DataArray(
            np.where(data.band < 5, "red", "blue"), 
            coords=[("band", data.band.values)]
        )
    
    styled_bands = style_bands(
        bands_data, {"color": color, "width": 3},
        spindown_style={"opacity": 0.5, "color": "blue"}
    )
