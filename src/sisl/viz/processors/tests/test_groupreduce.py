...
def dataarray():
    return xr.DataArray([
        ...
        [7, 8, 9],
        [10, 11, 12],
        ], coords=[("x", [0,1,2,3]), ("y", [0,1,2])], name="vals"
    )
...
def test_dataarray(dataarray):
    ...
    new = group_reduce(dataarray, [{"selector": [0,1]}, {"selector": [2, 3]}], reduce_dim="x", reduce_func=np.sum, groups_dim="selection")
...
def test_dataarray_multidim(dataarray):
    ...
    new = group_reduce(dataarray, [{"selector": ([0,1], [0,1])}, {"selector": ([2, 3], [0,1])}], reduce_dim=("x", "y"), reduce_func=np.sum, groups_dim="selection")
...
def test_dataarray_multidim_multireduce(dataarray):
    ...
    new = group_reduce(dataarray, [{"selector": ([0,1], [0,1])}, {"selector": ([2, 3], [0,1])}], reduce_dim=("x", "y"), 
        reduce_func=(np.sum, np.mean), groups_dim="selection")
...
def test_dataarray_sangroup(dataarray):
    ...
    new = group_reduce(dataarray, [{"selector": [0,1]}, {"selector": [2, 3]}], 
        reduce_dim="x", reduce_func=np.sum, groups_dim="selection",
...
def test_dataarray_names(dataarray):
    ...
    new = group_reduce(dataarray, [{"selector": [0,1], "name": "first"}, {"selector": [2, 3], "name": "second"}],
        reduce_dim="x", reduce_func=np.sum, groups_dim="selection")
...
def test_dataarray_groupvars(dataarray):
    ...
    new = group_reduce(dataarray, 
        [
            {"selector": [0,1], "name": "first", "color": "red", "size": 3}, 
            {"selector": [2, 3], "name": "second", "color": "blue", "size": 4}
        ], 
        reduce_dim="x", reduce_func=np.sum, groups_dim="selection", group_vars=["color", "size"]
    )
...
def test_dataarray_empty_selector(dataarray, drop):
    ...
    new = group_reduce(dataarray, [{"selector": []}, {"selector": [2, 3]}], reduce_dim="x", reduce_func=np.sum, groups_dim="selection", drop_empty=drop, fill_empty=0.)
    ...
    if not drop:
        assert new.sel(selection=0).sum() == 0.
    assert new.sel(selection=1).sum() == 7 + 8 + 9 + 10 + 11 + 12
...
def test_dataarray_empty_selector_0d():
    ...
    new = group_reduce(
        ...
        [{"selector": []}, {"selector": [1, 2]}], reduce_dim="x", reduce_func=np.sum, 
        groups_dim="selection", drop_empty=False, fill_empty=0.
    )
...
def test_dataset(dataset):
    ...
    new = group_reduce(dataset, [{"selector": [0,1]}, {"selector": [2, 3]}], reduce_dim="x", reduce_func=np.sum, groups_dim="selection")
...
def test_dataset_multidim(dataset):
    ...
    new = group_reduce(dataset, [{"selector": ([0,1], [0,1])}, {"selector": ([2, 3], [0,1])}], 
        reduce_dim=("x", "y"), reduce_func=np.sum, groups_dim="selection")
...
def test_dataset_multidim_multireduce(dataset):
    ...
    new = group_reduce(dataset, [{"selector": ([0,1], [0,1])}, {"selector": ([2, 3], [0,1])}], 
        reduce_dim=("x", "y"), reduce_func=(np.sum, np.mean), groups_dim="selection")
...
def test_dataset_names(dataset):
    ...
    new = group_reduce(dataset, [{"selector": [0,1], "name": "first"}, {"selector": [2, 3], "name": "second"}], reduce_dim="x", reduce_func=np.sum, groups_dim="selection")
...
def test_dataset_empty_selector(dataset, drop):
    ...
    new = group_reduce(dataset, [{"selector": []}, {"selector": [2, 3]}], reduce_dim="x", reduce_func=np.sum, groups_dim="selection", drop_empty=drop, fill_empty=0.)
