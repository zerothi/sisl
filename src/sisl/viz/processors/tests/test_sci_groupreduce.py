...
def atom_x():
    ...
    return xr.DataArray(geom.xyz[:, 0], coords=[("atom", range(geom.na))], attrs={"geometry": geom})
...
def test_reduce_atom_dataarray(atom_x):
    ...
    grouped = reduce_atom_data(
        ...
        reduce_func=np.sum, groups_dim="group"
    )
...
def test_reduce_atom_dataarray_cat(atom_x):
    ...
    grouped = reduce_atom_data(
        atom_x,
        [{"atoms": {"x": (0, 10)}, "name": "first"}, {"atoms": {"x": (10, None)}, "name": "second"}],
        reduce_func=np.max, groups_dim="group"
    )
...
def test_reduce_atom_cat_nogeom(atom_x):
    ...
    grouped = reduce_atom_data(
        atom_x,
        [{"atoms": {"x": (0, 10)}, "name": "first"}, {"atoms": {"x": (10, None)}, "name": "second"}],
        geometry=geometry, reduce_func=np.max, groups_dim="group"
    )
...
def test_reduce_atom_dataset_cat(atom_xyz):
    ...
    grouped = reduce_atom_data(
        atom_xyz,
        [{"atoms": {"x": (0, 10), "y": (1, 3)}, "name": "first"}, {"atoms": {"x": (10, None)}, "name": "second"}],
        reduce_func=np.max, groups_dim="group"
    )
