...
def transform_grid_cell(grid: Grid, cell: npt.NDArray[np.float_] = np.eye(3), output_shape: Optional[Tuple[int, int, int]] = None, mode: str = "constant", order: int = 1, **kwargs) -> Grid:
...
def orthogonalize_grid(grid:Grid, interp: Tuple[int, int, int] = (1, 1, 1), mode: str = "constant", **kwargs) -> Grid:
    ...
    **kwargs:
    ...
    return transform_grid_cell(
        grid, mode=mode, output_shape=tuple(interp[i] * grid.shape[i] for i in range(3)), cval=np.nan, **kwargs
    )

def orthogonalize_grid_if_needed(grid: Grid, axes: Sequence[str], tol: float = 1e-3, 
    interp: Tuple[int, int, int] = (1, 1, 1), mode: str = "constant", **kwargs) -> Grid:
...
def reduce_grid(grid: Grid, reduce_method: Literal["average", "sum"], keep_axes: Sequence[int]) -> Grid:
    ...
    for ax, (ax_range, direction) in enumerate(zip(ax_ranges, directions)):
        if ax_range is not None:
            ...
            if not is_1D_cartesian(cell, direction, tol=cart_tol):
                raise ValueError(f"Cannot sub grid along '{direction}', since there is no unique lattice vector that represents this direction. Cell: {cell}")
...
def interpolate_grid(grid: Grid, interp: Tuple[int, int, int] = (1, 1, 1), force: bool = False) -> Grid:
...
def grid_geometry(grid: Grid, geometry: Optional[Geometry] = None) -> Union[Geometry, None]:
...
def grid_to_dataarray(grid: Grid, axes: Sequence[str], grid_axes: Sequence[int], nsc: Tuple[int, int, int]) -> GridDataArray:
