...
def expected_neighs(sphere_overlap, multiR, self_interaction, pbc):
    ...
    return np.array(first_at_neighs), np.array(second_at_neighs), np.array(third_at_neighs)
...
def test_neighbour_pairs(neighfinder, self_interaction, pbc, expected_neighs):
    ...
    neighs = neighfinder.find_neighbours(as_pairs=True, self_interaction=self_interaction, pbc=pbc)
...
def test_neighbours_lists(neighfinder, self_interaction, pbc, expected_neighs):
    ...
    neighs = neighfinder.find_neighbours(as_pairs=False, self_interaction=self_interaction, pbc=pbc)
    ...
    # Check shapes
    for i, i_at_neighs in enumerate([first_at_neighs, second_at_neighs, third_at_neighs]):
        assert neighs[i].shape == (len(i_at_neighs), 4), f"Wrong shape for neighbours of atom {i}"
    ...
    for i, i_at_neighs in enumerate([first_at_neighs, second_at_neighs, third_at_neighs]):
        ...
        assert np.all(neighs[i] == i_at_neighs[:, 1:]), f"Wrong values for neighbours of atom {i}"
...
def test_all_unique_pairs(neighfinder, self_interaction, pbc, expected_neighs):
    ...
    neighs = neighfinder.find_all_unique_pairs(self_interaction=self_interaction, pbc=pbc)

    first_at_neighs, second_at_neighs, third_at_neighs = expected_neighs

    all_expected_neighs = np.array([*first_at_neighs, *second_at_neighs, *third_at_neighs])
