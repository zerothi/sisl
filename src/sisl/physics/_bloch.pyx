...
def bloch_unfold(np.ndarray[np.int32_t, ndim=1, mode='c'] B,
    ...
    return _unfold(B, (k * 2 * pi).reshape(B[2], B[1], B[0], 3), M)
...
def _unfold(const int[::1] B, const double[:, :, :, ::1] k2pi,
    ...
    cdef np.ndarray[complexs_st, ndim=2, mode='c'] M = np.zeros([N * N1, N * N2],
                                                                dtype=dtype)
    cdef complexs_st[:, :, :, ::1] M4 = M.reshape(N, N1, N, N2)
    cdef complexs_st[:, ::1] M2 = M
    # Apparently, if we don't have a temporary, it fails... :(
    cdef const double[:] k2pi0 = k2pi[0, 0, :, 0]
    cdef const double[:] k2pi1 = k2pi[0, :, 0, 1]
    cdef const double[:] k2pi2 = k2pi[:, 0, 0, 2]

    # Split calculations into single expansion (easy to abstract)
    # and full calculation (which is too heavy!)
    if B0 == 1:
        if B1 == 1: # only B2 == 1
            _unfold_1(B2, k2pi2, N1, N2, m, M4)
        elif B2 == 1: # only B1 == 1
            _unfold_1(B1, k2pi1, N1, N2, m, M4)
        else:# only B0 == 1
            _unfold_2(B1, k2pi1, B2, k2pi2, N1, N2, m, M4)
    elif B1 == 1:
        if B2 == 1:
            _unfold_1(B0, k2pi0, N1, N2, m, M4)
        else:# only B1 == 1
            _unfold_2(B0, k2pi0, B2, k2pi2, N1, N2, m, M4)
    elif B2 == 1: # only B2 == 1
        _unfold_2(B0, k2pi0, B1, k2pi1, N1, N2, m, M4)
    else:
        _unfold_3(B0, B1, B2, k2pi, N1, N2, m, M2)
    ...
    for C in range(B2):
        for B in range(B1):
            for A in range(B0):
                ...
                k1 = k2pi[C, B, A, 1]
                k2 = k2pi[C, B, A, 2]
                _unfold_matrix(w, B0, B1, B2, k0, k1, k2, N1, N2, m[T], M)
                T = T + 1
