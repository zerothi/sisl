...
class PeriodicTable(object):
    ...
    def radii(self,key,radii='calc'):
        ...
        Z = self.Z_int(key)
        if radii == 'calc':
            if isinstance(Z,Integral): return self._radius_calc[Z] / 100
            return np.array([self._radius_calc[i] for i in Z],np.float64) / 100
        elif radii == 'empirical':
            if isinstance(Z,Integral): return self._radius_empirical[Z] / 100
            return np.array([self._radius_empirical[i] for i in Z],np.float64) / 100
        elif radii == 'vdw':
            if isinstance(Z,Integral): return self._radius_vdw[Z] / 100
            return np.array([self._radius_vdw[i] for i in Z],np.float64) / 100
        raise ValueError('Radii option could not be deciphered [calc|empirical|vdw].')
