...
def pre_prepare_sisl (frac_or_cart_or_index,Initial_Geom,InitialAtomPosition,FinalAtomPosition,rtol,atol,Ghost):
    ...
    if frac_or_cart_or_index == 'cartesian':
        ...
        if AtomIndex(XYZ,FinalAtomPosition,rtol,atol) < AtomIndex(XYZ,InitialAtomPosition,rtol,atol):
            ...
            InitialASEXYZ = InitialASEXYZ.remove(AtomIndex(XYZFull,FinalAtomPosition,rtol,atol))
            FinalASEXYZ = FinalASEXYZ.remove(AtomIndex(XYZFull,InitialAtomPosition,rtol,atol)-1)   
        ...
        if Ghost == True:
            ...
            #Ghost_initial_Info = sisl.Atom(Initial_Geom.atoms.Z[AtomIndex(Frac,InitialAtomPosition,rtol,atol)])
            #Ghost_initial = sisl.Geometry(Initial_Geom.xyz[AtomIndex(Frac,InitialAtomPosition,rtol,atol)],
            #                              atoms= sisl.Atom( -1* Ghost_initial_Info.atoms.Z , tag=Ghost_initial_Info.symbol+"_ghost"))
            #Ghost_final_Info = sisl.Atom(Initial_Geom.xyz[AtomIndex(Frac,FinalAtomPosition,rtol,atol)])
            #Ghost_final = sisl.Geometry(Initial_Geom.xyz[AtomIndex(Frac,FinalAtomPosition,rtol,atol)],
            #                            atoms=sisl.Atom( -1* Ghost_final_Info.atoms.Z,tag = Ghost_final_Info.symbol+"_ghost" ))
