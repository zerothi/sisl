...
__all__ = ['_k_R_factor_csr_f32', '_k_R_factor_csr_f64',
           '_k_R_factor_csr_c64', '_k_R_factor_csr_c128',
           '_k_R_factor_array_f32', '_k_R_factor_array_f64',
           '_k_R_factor_array_c64', '_k_R_factor_array_c128']
...
def _k_R_factor_csr_f32(np.ndarray[np.int32_t, ndim=1, mode='c'] PTR,
    ...
    for r in range(nr):
        for ind in range(ptr[r], ptr[r] + ncol[r]):
            ...
            vz[v_ptr[r] + s_idx] = vz[v_ptr[r] + s_idx] + d * fac[s, 2]
...
@cython.initializedcheck(False)
@cython.cdivision(True)
def _k_R_factor_csr_f64(np.ndarray[np.int32_t, ndim=1, mode='c'] PTR,
                        np.ndarray[np.int32_t, ndim=1, mode='c'] NCOL,
                        np.ndarray[np.int32_t, ndim=1, mode='c'] COL,
                        numeric_real[:, ::1] D, const int idx,
                        np.ndarray[np.float64_t, ndim=2, mode='c'] FAC):

    # Convert to memory views
    cdef int[::1] ptr = PTR
    cdef int[::1] ncol = NCOL
    cdef int[::1] col = COL
    cdef double[:, ::1] fac = FAC
...
@cython.initializedcheck(False)
@cython.cdivision(True)
def _k_R_factor_csr_c64(np.ndarray[np.int32_t, ndim=1, mode='c'] PTR,
                        np.ndarray[np.int32_t, ndim=1, mode='c'] NCOL,
                        np.ndarray[np.int32_t, ndim=1, mode='c'] COL,
                        numeric_complex[:, ::1] D, const int idx,
                        np.ndarray[np.complex64_t, ndim=2, mode='c'] FAC):

    # Convert to memory views
    cdef int[::1] ptr = PTR
    cdef int[::1] ncol = NCOL
    cdef int[::1] col = COL
    cdef float complex[:, ::1] fac = FAC
...
@cython.initializedcheck(False)
@cython.cdivision(True)
def _k_R_factor_csr_c128(np.ndarray[np.int32_t, ndim=1, mode='c'] PTR,
                         np.ndarray[np.int32_t, ndim=1, mode='c'] NCOL,
                         np.ndarray[np.int32_t, ndim=1, mode='c'] COL,
                         numeric_complex[:, ::1] D, const int idx,
                         np.ndarray[np.complex128_t, ndim=2, mode='c'] FAC):

    # Convert to memory views
    cdef int[::1] ptr = PTR
    cdef int[::1] ncol = NCOL
    cdef int[::1] col = COL
    cdef double complex[:, ::1] fac = FAC
...
@cython.initializedcheck(False)
@cython.cdivision(True)
def _k_R_factor_array_f32(np.ndarray[np.int32_t, ndim=1, mode='c'] PTR,
                          np.ndarray[np.int32_t, ndim=1, mode='c'] NCOL,
                          np.ndarray[np.int32_t, ndim=1, mode='c'] COL,
                          numeric_real[:, ::1] D, const int idx,
                          np.ndarray[np.float32_t, ndim=2, mode='c'] FAC):
    ...
    for r in range(nr):
        for ind in range(ptr[r], ptr[r] + ncol[r]):
            ...
            s = col[ind] / nr
            d = <float> D[ind, idx]
            vx[r, c] = vx[r, c] + d * fac[s, 0]
            vy[r, c] = vy[r, c] + d * fac[s, 1]
            vz[r, c] = vz[r, c] + d * fac[s, 2]
...
@cython.initializedcheck(False)
@cython.cdivision(True)
def _k_R_factor_array_f64(np.ndarray[np.int32_t, ndim=1, mode='c'] PTR,
                          np.ndarray[np.int32_t, ndim=1, mode='c'] NCOL,
                          np.ndarray[np.int32_t, ndim=1, mode='c'] COL,
                          numeric_real[:, ::1] D, const int idx,
                          np.ndarray[np.float64_t, ndim=2, mode='c'] FAC):
...
@cython.initializedcheck(False)
@cython.cdivision(True)
def _k_R_factor_array_c64(np.ndarray[np.int32_t, ndim=1, mode='c'] PTR,
                          np.ndarray[np.int32_t, ndim=1, mode='c'] NCOL,
                          np.ndarray[np.int32_t, ndim=1, mode='c'] COL,
                          numeric_complex[:, ::1] D, const int idx,
                          np.ndarray[np.complex64_t, ndim=2, mode='c'] FAC):
...
@cython.initializedcheck(False)
@cython.cdivision(True)
def _k_R_factor_array_c128(np.ndarray[np.int32_t, ndim=1, mode='c'] PTR,
                           np.ndarray[np.int32_t, ndim=1, mode='c'] NCOL,
                           np.ndarray[np.int32_t, ndim=1, mode='c'] COL,
                           numeric_complex[:, ::1] D, const int idx,
                           np.ndarray[np.complex128_t, ndim=2, mode='c'] FAC):
