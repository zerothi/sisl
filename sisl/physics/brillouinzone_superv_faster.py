...
class SupervisedMethod:
    ...
    def __init__(self, method, really_executing):

        self.method = method
        self.really_executing = really_executing

    def __call__(self):

        return self.really_executing()

    def __getattribute__(self, attr):

        real_getattr = object.__getattribute__

        if attr not in ["method", "really_executing", "__call__", "__init__"]:
            return real_getattr(self.method, attr)
        else:
            return real_getattr(self, attr)
...
class Supervisable:
    ...
    def __getattribute__(self, attr):
        ...
        obj = real_getattr(self, "obj")

        try:
            val = real_getattr(self, attr)
        except:
            val = getattr(obj, attr)

        if callable(val) and attr[0] != "_":
            return SupervisedMethod(val, partial(real_getattr(self, "__supervise__"), obj, val))
        else:
            return val

    @staticmethod
    def __supervise__(self, method, *args, **kwargs):
        return method(*args, **kwargs)

    def _unsupervise(self):
        '''
        Only works if the object has been made supervisable.
        (Not if it is supervisable because of class inheritance)
        '''
        return self.obj  
...
class BrillouinZone:
    ...
    def __init__(self, parent, k=None, weight=None):
        ...
        if len(self.k) != len(self.weight):
            raise ValueError(self.__class__.__name__ +
                             '.__init__ requires input k-points and weights to be of equal length.')
    ...
    def set_parent(self, parent):
        ...
        try:
            ...
            parent = SuperCell(parent)

        self.parent = Supervisable(parent)
    ...
    def param_circle(self, sc, N_or_dk, kR, normal, origo, loop=False):
        ...
        if isinstance(N_or_dk, Integral):
            ...
            if N < 4:
                N = 4
                info(
                    'BrillouinZone.param_circle increased the number of circle points to 4.')
        ...
        if theta != 0:
            ...
            pv /= fnorm(pv)
            q = Quaternion(phi, pv, rad=True) * \
                Quaternion(theta, [0, 0, 1], rad=True)
        else:
    ...
    def asarray(self):
        ...
        def _call(self, *args, **kwargs):
            ...
            if has_wrap:
                wrap = allow_kwargs('parent', 'k', 'weight')(
                    kwargs.pop('wrap'))
            ...
            w = self.weight
            if has_wrap:
                v = wrap(func(*args, k=k[0], **kwargs),
                         parent=parent, k=k[0], weight=w[0])
            else:
            ...
            if has_wrap:
                for i in range(1, len(k)):
                    a[i] = wrap(func(*args, k=k[i], **kwargs),
    ...
    def asaverage(self):
        ...
        def _asaverage(parent, method, *args, **kwargs):
            ...
            w = self.weight
            if has_wrap:
                v = wrap(method(*args, k=k[0], **kwargs),
                        parent=parent, k=k[0], weight=w[0]) * w[0]
                eta.update()
                for i in range(1, len(k)):
                    v += wrap(method(*args, k=k[i], **kwargs),
                            parent=parent, k=k[i], weight=w[i]) * w[i]
                    eta.update()
            else:
        ...
        # Set instance __call__
        setattr(self.parent, '__supervise__', _asaverage)
...
class MonkhorstPack(BrillouinZone):
    ...
    def __init__(self, parent, nkpt, displacement=None, size=None, centered=True, trs=True):
        ...
        if np.any(nkpt - np.diag(np.diag(nkpt)) != 0):
            raise NotImplementedError(
                self.__class__.__name__ + " with off-diagonal components is not implemented yet")
        ...
        kw = [self.grid(Dn[i], displacement[i], size[i],
                        centered, i == i_trs) for i in (0, 1, 2)]
        ...
        if trs:
            ...
            if abs(kw[i_trs][0][0]) < 1e-10:
                ...
                k_dup = k_dup.reshape(
                    k1.size, k2.size, 1, 1, 2) + k_dup.reshape(1, 1, k1.size, k2.size, 2)
                k_dup = ((k_dup[..., 0] ** 2 + k_dup[..., 1]
                          ** 2) ** 0.5 < 1e-10).nonzero()
                ...
                if kw[ik1][0].size == 1 and kw[ik1][0][0] == 0.:
                    ...
                    rel = np.logical_and(
                        k_dup[0] > k_dup[2], k_dup[1] > k_dup[3])
                k_dup = (k_dup[0][rel], k_dup[1][rel],
                         k_dup[2][rel], k_dup[3][rel])
                del rel, k1, k2
            else:
    ...
    def copy(self):
        """ Create a copy of this object """
        bz = self.__class__(self.parent, self._diag, self._displ,
                            self._size, self._centered, self._trs >= 0)
        bz._k = self._k.copy()
        bz._w = self._w.copy()
        return bz
    ...
    def asgrid(self):
        ...
        def _call(self, *args, **kwargs):
            ...
            if not np.all(self._displ == 0):
                raise SislError(
                    self.__class__.__name__ + f'.{self._bz_attr} requires the displacement to be 0 for all k-points.')
            ...
            if grid_unit == 'b':
                ...
                cell = parent.sc.rcell * \
                    self._size.reshape(1, -1) / units('Ang', grid_unit)
            ...
            if data_axis is None:
                if v.size != 1:
                    raise SislError(
                        self.__class__.__name__ + f'.{self._bz_attr} requires one value per-kpoint because of the 3D grid values')
    ...
    def replace(self, k, mp):
        ...
        if len(idx) == 0:
            raise SislError(self.__class__.__name__ +
                            '.reduce could not find any points to replace.')
        ...
        if abs(total_weight - replace_weight) < 1e-8:
            ...
            if self._trs < 0:
                info(self.__class__.__name__ +
                     '.reduce assumes that the replaced k-point has double weights.')
        else:
            ...
            print(self.k[idx, :])
            raise SislError(self.__class__.__name__ +
                            '.reduce could not assert the weights are consistent during replacement.')
...
class BandStructure(BrillouinZone):
    ...
    def __init__(self, parent, point, division, name=None):
        ...
        if self.point.shape[1] < 3:
            if self.point.shape[1] != np.sum(self.parent.nsc > 1):
                raise ValueError(
                    'Could not determine the non-periodic direction')
    ...
    def lineark(self, ticks=False):
        ...
        for i in range(len(dk)):
            ...
            dK[xtick[i]:xtick[i+1]
               ] = ls(k_len[i], k_len[i+1], n, dtype=np.float64, endpoint=end)
        xtick[-1] -= 1
