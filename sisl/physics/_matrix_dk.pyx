...
def _matrix_dk_R(csr, const int idx, sc,
    ...
    iRs = -1j * _dot(sc.sc_off, sc.cell).astype(dtype, copy=False)
    if is_gamma(k):
    ...
    if dtype == np.complex128:
        ...
        if format == 'array':
            return _dk_R_array_c128(csr.ptr, csr.ncol, csr.col, csr._D, idx, iRs, phases)
        elif format == 'matrix' or format == 'dense':
            d1, d2, d3 = _dk_R_array_c128(csr.ptr, csr.ncol, csr.col, csr._D, idx, iRs, phases)
            return np.asmatrix(d1), np.asmatrix(d2), np.asmatrix(d3)
        
        # Default must be something else.
        d1, d2, d3 = _dk_R_csr_c128(csr.ptr, csr.ncol, csr.col, csr._D, idx, iRs, phases)
        return d1.asformat(format), d2.asformat(format), d3.asformat(format)
    
    elif dtype == np.complex64:
        if format == 'array':
            return _dk_R_array_c64(csr.ptr, csr.ncol, csr.col, csr._D, idx, iRs, phases)
        elif format == 'matrix' or format == 'dense':
            d1, d2, d3 = _dk_R_array_c64(csr.ptr, csr.ncol, csr.col, csr._D, idx, iRs, phases)
            return np.asmatrix(d1), np.asmatrix(d2), np.asmatrix(d3)
        d1, d2, d3 = _dk_R_csr_c64(csr.ptr, csr.ncol, csr.col, csr._D, idx, iRs, phases)
        return d1.asformat(format), d2.asformat(format), d3.asformat(format)
